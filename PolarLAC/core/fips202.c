 ...
 *
 * Arguments:   - uint64_t *s: pointer to Keccak state
 *              - uint32_t pos: position in current block to be absorbed
 ...
 **************************************************/
static uint32_t keccak_absorb(uint64_t s[25], uint32_t pos,
								  uint32_t r, const uint8_t *in,
								  size_t inlen)
{
	uint32_t i;
 ...
 **************************************************/
static void keccak_finalize(uint64_t s[25], uint32_t pos, uint32_t r,
							uint8_t p)
{
 ...
 *              - uint32_t pos: number of bytes in current block already
 *squeezed
 *              - uint32_t r: rate in bytes (e.g., 168 for SHAKE128)
 *
 * Returns new position pos in current block
 **************************************************/
static uint32_t keccak_squeeze(uint8_t *out, size_t outlen, uint64_t s[25],
 ...
 *functions
 **************************************************/
static void keccak_absorb_once(uint64_t s[25], uint32_t r,
...
{
	OP_hash(OP_ALG_SHAKE128, OP_MODE_NORMAL, (int32_t)outlen, (void *)in, (int32_t)inlen, 0, out);
}
...
{
	OP_hash(OP_ALG_SHAKE256, OP_MODE_NORMAL, (int32_t)outlen, (void *)in, (int32_t)inlen, 0, out);
}
...
{
	OP_hash(OP_ALG_SHA3_256, OP_MODE_NORMAL, 32, (void *)in, (int32_t)inlen, 0, h);
}
#else
void sha3_256(uint8_t h[32], const uint8_t *in, size_t inlen)
{
...
{
	OP_hash(OP_ALG_SHA3_512, OP_MODE_NORMAL, 64, (void *)in, (int32_t)inlen, 0, h);
}
#else
void sha3_512(uint8_t h[64], const uint8_t *in, size_t inlen)
{	
...
#endif
void scloudplus_F(unsigned char *output, uint64_t outlen,
				  const unsigned char *input, uint64_t inlen)
{
	shake256(output, outlen, input, inlen);
}
void scloudplus_K(unsigned char *output, uint64_t outlen,
				  const unsigned char *input, uint64_t inlen)
{
	shake256(output, outlen, input, inlen);
}
