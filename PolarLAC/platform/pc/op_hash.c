...
#define CSR_IDX 25
#define GET_POS(s)       ((uint32_t)((s)[25] & 0x7FFFFFFF))
...
static int32_t get_alg_params(uint8_t alg, uint32_t *rate, uint8_t *padding) {
    switch (alg) {
...
static uint32_t oph_absorb(uint64_t s[25], uint32_t pos,
...
static void oph_finalize(uint64_t s[25], uint32_t pos, uint32_t r, uint8_t p) {
    s[pos / 8] ^= (uint64_t)p << 8 * (pos % 8);
    s[r / 8 - 1] ^= 1ULL << 63;
}

static uint32_t oph_squeeze(uint8_t *out, size_t outlen, uint64_t s[25],
...
static void oph_absorb_once(uint64_t s[25], uint32_t r,
