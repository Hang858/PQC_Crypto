...
#define SM3_W_WORDS 68
#define SM3_W1_WORDS 64
...
*******************************************************************************/
void BigEndian(unsigned char src[], uint32_t bytelen, unsigned char des[])
{
    unsigned char tmp = 0;
    uint32_t i = 0;
...
{
    uint32_t *W = malloc(sizeof(uint32_t) * (SM3_W_WORDS + SM3_W1_WORDS));
    uint32_t *W1;
    if (W == NULL) {
        return;
    }
...
int32_t len //bytelen of message
Output: SM3_STATE *md
Return: null
Others:
*******************************************************************************/
void SM3_process(SM3_STATE *md, unsigned char *buf, int32_t len)
{
    while (len--)
    {
        /* copy byte */
...
int32_t len //bytelen of the message
Output: unsigned char hash[32]
Return: null
Others:
*******************************************************************************/
void SM3_256(unsigned char* buf, int32_t len, unsigned char* hash)
{
    SM3_STATE md;
    SM3_init(&md);
