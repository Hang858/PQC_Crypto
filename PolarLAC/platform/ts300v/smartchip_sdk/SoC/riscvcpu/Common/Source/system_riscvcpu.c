 ...
 */
/******************************************************************************
 * @file     system_riscvcpu.c
 * @brief    RMSIS Smartchip Core Device Peripheral Access Layer Source File for
...
/**
 * \defgroup  RMSIS_Core_SystemConfig       System Device Configuration
 * \brief Functions for system and clock setup available in system_<device>.c.
 * \details
 * Smartchip provides a template file **system_Device.c** that must be adapted by
 ...
 *  -  Vendor customized interrupt, exception and nmi handling code, see \ref RMSIS_Core_IntExcNMI_Handling
 *
...
/**
 * \defgroup  RMSIS_Core_IntExcNMI_Handling   Interrupt and Exception and NMI Handling
 * \brief Functions for interrupt, exception and nmi handle available in system_<device>.c.
 * \details
 * Smartchip provide a template for interrupt, exception and NMI handling. Silicon Vendor could adapat according
 ...
 * the exception codes Smartchip RV/RVX core provided.
...
void Exception_DumpFrame(unsigned long sp)
{
    ...
    printf("ra: 0x%lx, tp: 0x%lx, t0: 0x%lx, t1: 0x%lx, t2: 0x%lx, t3: 0x%lx, t4: 0x%lx, t5: 0x%lx, t6: 0x%lx\n" \
           ...
           exc_frame->a6, exc_frame->a7, exc_frame->mcause, exc_frame->mepc);
#else
    printf("ra: 0x%lx, tp: 0x%lx, t0: 0x%lx, t1: 0x%lx, t2: 0x%lx\n" \
           ...
           exc_frame->a4, exc_frame->a5, exc_frame->mcause, exc_frame->mepc);
#endif
}
...
/** @} */ /* End of Doxygen Group RMSIS_Core_ExceptionAndNMI */

/** Banner Print for Smartchip SDK */
void SystemBannerPrint(void)
{
#if defined(SMARTCHIP_BANNER) && (SMARTCHIP_BANNER == 1)
...
#error DOWNLOAD_MODE is not defined via build system, please check!
    ...
    printf("Smartchip SDK Build Time: %s, %s\r\n", __DATE__, __TIME__);
...
/**
 * \brief initialize clic config
 * \details
 * CLIC needs be initialized after boot up,
 * Vendor could also change the initialization
 * configuration.
 */
void CLIC_Init(void)
{
    /* Global Configuration about ILT and NLBits.
     * TODO: Please adapt it according to your system requirement.
     * This function is called in _init function */
    CLIC_SetIlt(0);
    CLIC_SetCfgNlbits(__CLIC_INTCTLBITS);
}
 ...
 * \param [in]  trig_mode   see \ref CLIC_TRIGGER_Type
 * \param [in]  lvl         interupt level
 * \param [in]  priority    interrupt priority
 * \param [in]  handler     interrupt handler, if NULL, handler will not be installed
 * \return       -1 means invalid input parameter. 0 means successful.
 * \remarks
 * - This function use to configure specific clic interrupt and register its interrupt handler and enable its interrupt.
 * - If the vector table is placed in read-only section(FLASHXIP mode), handler could not be installed
 */
int32_t CLIC_Register_IRQ(IRQn_Type IRQn, CLIC_TRIGGER_Type trig_mode, uint8_t lvl, uint8_t priority, void *handler)
{
    if ((IRQn > SOC_INT_MAX) || (trig_mode > CLIC_NEGTIVE_EDGE_TRIGGER )) {
        return -1;
    }
    ...
    if (handler != NULL) {
        ...
        CLIC_SetVector(IRQn, (rv_csr_t)handler);
    }
    /* enable interrupt */
    CLIC_EnableIRQ(IRQn);
    return 0;
...
void _premain_init(void)
{
    ...
    /* __ICACHE_PRESENT and __DCACHE_PRESENT are defined in riscvcpu.h */
    ...
    Exception_Init();
    /* CLIC initialization, mainly ILT and NLBIT */
    CLIC_Init();
    printf("Current icache %d, dcache %d, ccm %d\n", \
            __ICACHE_PRESENT, __DCACHE_PRESENT, __CCM_PRESENT);
}
...
/** @} */ /* End of Doxygen Group RMSIS_Core_SystemAndClock */
