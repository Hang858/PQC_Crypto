...
/**
 * @brief Radix conversion for large polynomials in GF(2^m)[x].
 *
 * Generalized radix step when the polynomial size 2^m_f exceeds
 * the small-case thresholds (handled in radix()).
 *
 * @param[out] f0  Lower-half output array of size 2^{m_f-1}
 * @param[out] f1  Upper-half output array of size 2^{m_f-1}
 * @param[in]  f   Input array of size 2^{m_f}
...
/**
 * @brief Retrieves the error polynomial error from the evaluations w of the ELP (Error Locator Polynomial) on all field
 * elements.
 *
