...
uint32_t sha3_init(uint8_t alg, void *s, uint32_t s_len)
{
	...
	//Support multiple squeeze and first to clr start
    sha3_reg_write(SCCFG_OFFSET, SCCFG_SHA3_CLR);
    while(0!= sha3_reg_read(SCCFG_OFFSET));
    //Support multiple squeeze and first to clr end
...
uint32_t sha3_squeeze(uint8_t alg, void *s, uint32_t s_len, uint8_t *output, uint32_t output_len)
{
	...
	while(output_len>0)
	{
		    ...
		    if (pCtx->pos >= params.block_size)
		    {
		    	if(0x5a != firstSqueeze)
		    	{
					sha3_reg_write(SCSR_OFFSET, 0);
		    	...
		    	reg_blocks = params.block_size / 4U;
				for (uint8_t i = 0; i < reg_blocks; i++) {
					uint32_t reg_data = sha3_reg_read(SCDR_OFFSET);
					*(uint32_t *)(pCtx->buf + i*4) = reg_data;
				}
		    ...
		    uint8_t remain_in_cache = params.block_size - pCtx->pos;
		    uint8_t copy_len = (remain_in_cache < output_len) ? remain_in_cache : output_len;
		    memcpy(output,(pCtx->buf + pCtx->pos),copy_len);
